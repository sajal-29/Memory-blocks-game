...
char textd[] = { "MAA KI LODI" };

char textm[] = { "Madhur Gaba" };

char textf[] = { "Sanchi Batra" };

char texte[] = { "Sajal Jain" };

char textg[] = { "102116082" };

char textnum[] = { "102116093" };

char texth[] = { "102116080" };

char texti[] = { "name" };

char textj[] = { "Department of CSE" };

char textk[] = { "Computer Graphics" };
...
void display(void)    //display function
{
	...
	if (k == 0)
	{
		...
		for (j = 0; textnum[j] != '\0'; j++)
			glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, textnum[j]);


		glRasterPos3f(-68, -4, 0);
		for (j = 0; texth[j] != '\0'; j++)
			glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, texth[j]);


		glRasterPos3f(88, 18, 0);
		for (j = 0; textd[j] != '\0'; j++)
